...
import sendPushNotification, { generatePushNotificationMessge } from "../utils/ExpoSendPushNotification.js";
...
const saveReservationMessages = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        await Notification.create({
            ...
            content: `You have received a new message from ${senderId === reservation.providerId._id.toString() ? reservation.providerId.name : reservation.askerId.name}.`,
        });

        // Send push notification
        const pushMessage = generatePushNotificationMessge(
            "New Message",
            `You have received a new message from ${senderId === reservation.providerId._id.toString() ? reservation.providerId.name : reservation.askerId.name}.`,
            recipient.pushToken
        );
...
const createPayment = asyncHandler(async (req, res) => {
    try {
        ...
        // Create a notification
        const notificationMessage = generateNotificationContentForPayment(req.body.askerName, req.body.amount, req.body.providerName);
        await Notification.create({
...
const getAllFavourites = asyncHandler(async (req, res) => {
    try {
        const favourites = await UserFavourite.findOne({ userId: req.params.userId }).populate('Favourites.offer');
